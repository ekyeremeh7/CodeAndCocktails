...
class AnalyticsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (userData == null || userData!.results.isEmpty) {
      return Scaffold(
        ...
        body: const Center(child: Text('No data available for analytics')),
      );
    }
  ...
  Widget _buildTicketTypesPieChart(BuildContext context, UserResponse data) {
    ...
    final colorList = ticketTypeEntries
        .map((entry) => typeColors[entry.key]!)
        .toList();
  ...
  Widget _buildTopInsights(BuildContext context, UserResponse data) {
    ...
    final topTicketType = sortedTypes.isNotEmpty
        ? sortedTypes.first.key
        : 'N/A';
    final topBuyerEmail = sortedBuyers.isNotEmpty
        ? sortedBuyers.first.key
        : 'N/A';
    final topBuyerCount = sortedBuyers.isNotEmpty
        ? sortedBuyers.first.value
        : 0;
